...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m vllm.entrypoints.openai.api_server --served-model-name qwen-vl --model /data1/sjw_data/ARPO/Qwen-VL --port 8088 --tensor-parallel-size 4 --gpu-memory-utilization 0.85 --limit-mm-per-prompt "image=5" --disable-custom-all-reduce