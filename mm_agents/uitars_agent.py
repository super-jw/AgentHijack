    ...
    for action_instance, raw_str in zip(parsed_actions, all_action):
        ...
        for param_name, param in params.items():
            ...
            if "start_box" in param_name or "end_box" in param_name:
                ...
                numbers = ori_box.replace("<bbox>","").replace("</bbox>","").replace("(", "").replace(")", "").split(",")
...
class UITARSAgent:
    ...
    def predict(
        self, instruction: str, obs: Dict, last_action_after_obs: Dict = None, env=None, example=None,
    ) -> List:
        """
        ...
        if self.observation_type in ["screenshot", "screenshot_a11y_tree"]:
            ...
            if not is_single_color_image(current_observation):
                if self.noise_type not in ["clean", "initialization_error", "network_error"]:
        ...
        while True:
            ...
            try:
                response = self.vlm.chat.completions.create(model=self.model, messages=messages, frequency_penalty=1, max_tokens=self.max_tokens, temperature=temperature, top_p=self.top_p)
