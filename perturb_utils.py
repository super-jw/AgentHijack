...
def extract_bounding_boxes_from_image(image_input):
    ...
    # Check if the input is already in bytes, if not, convert it
    if isinstance(image_input, bytes):
        ...
        image_input.save(image_bytes_io, format='PNG')
    ...
    try:
        # Perform OCR on the image bytes with a timeout
        poller = document_analysis_client.begin_analyze_document(
            "prebuilt-read", document=image_bytes
        )
        # Wait for the result with a timeout (in seconds)
        result = poller.result(timeout=30)
    except HttpResponseError as e:
        print(f"HTTP response error during OCR processing: {e}")
        return boxes
    except ServiceRequestError as e:
        print(f"Service request error during OCR processing: {e}")
        return boxes
    except ClientAuthenticationError as e:
        print(f"Authentication error during OCR processing: {e}")
        return boxes
    except AzureError as e:
        print(f"Azure error during OCR processing: {e}")
        return boxes
    except Exception as e:
        print(f"An unexpected error occurred during OCR processing: {e}")
        return boxes

    # Extract bounding boxes from the OCR result
    try:
        ...
        print(f"An error occurred during bounding box extraction: {e}")
...
def is_valid_position(new_bbox, existing_bboxes):
    for bbox in existing_bboxes:
        if is_overlapping(new_bbox, bbox):
            return False
    return True

def generate_star_points(center_x, center_y, outer_radius, inner_radius, num_points=5):
    points = []
    for i in range(10):
...
def scale_actions(action: str, scale: float) -> str:
    def replace_move(match):
        x = float(match.group(1))
        y = float(match.group(3))
        return f'moveTo({int(x * scale)}, {int(y * scale)}'
    ...
    def replace_click(match):
        x = float(match.group(1))
        y = float(match.group(3))
        return f'lick({int(x * scale)}, {int(y * scale)}'
    
    def replace_scroll_up(match):
        x = float(match.group(1))
        y = float(match.group(3))
        return f'scroll(5, x={int(x * scale)}, y={int(y * scale)}'
    
    def replace_scroll_down(match):
        x = float(match.group(1))
        y = float(match.group(3))
        return f'scroll(-5, x={int(x * scale)}, y={int(y * scale)}'

    action = re.sub(r'moveTo\((\d+(\.\d+)?), \s*(\d+(\.\d+)?)', replace_move, action)
    action = re.sub(r'dragTo\((\d+(\.\d+)?), \s*(\d+(\.\d+)?)', replace_drag, action)
    action = re.sub(r'lick\((\d+(\.\d+)?), \s*(\d+(\.\d+)?)', replace_click, action)
    action = re.sub(r'scroll\(5, x=(\d+(\.\d+)?), y=(\d+(\.\d+)?)', replace_scroll_up, action)
    action = re.sub(r'scroll\(-5, x=(\d+(\.\d+)?), y=(\d+(\.\d+)?)', replace_scroll_down, action)
